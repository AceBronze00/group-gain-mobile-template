...
import LateJoinerModal from "./LateJoinerModal";
...
const JoinGroupModal = ({ open, onOpenChange }: JoinGroupModalProps) => {
  const { joinGroup, calculateLateJoinerAmount } = useApp();
  const [groupCode, setGroupCode] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [showLateJoinerModal, setShowLateJoinerModal] = useState(false);
  const [lateJoinerAmount, setLateJoinerAmount] = useState(0);
  ...
  const handleJoinGroup = async () => {
    ...
    try {
      ...
      if (result && result.isLateJoiner) {
        setLateJoinerAmount(result.requiredAmount);
  ...
  const handleLateJoinerConfirm = async () => {
    setIsLoading(true);
    try {
      await joinGroup(groupCode.trim().toUpperCase(), lateJoinerAmount);
      setShowLateJoinerModal(false);
      onOpenChange(false);
      setGroupCode('');
    } catch (error) {
      console.error('Error joining group as late joiner:', error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <ul className="text-xs text-blue-600 space-y-1">
                  ...
                  <li>• You can join groups even after they've started</li>
                  <li>• Late joiners may need to make a catch-up payment</li>
      ...
      <LateJoinerModal
        open={showLateJoinerModal}
        onOpenChange={setShowLateJoinerModal}
        groupCode={groupCode}
        requiredAmount={lateJoinerAmount}
        onConfirm={handleLateJoinerConfirm}
      />
    </>
  );
};
